...
<Row>
  <Col>
    ...
    Stores are owned by a single [user](user).
...
<Row>
  <Col>
    ...
    Members are identified by the underlying [user](user)'s ID.
...
<Row>
  <Col>
    Creates a new store owned by the currently authenticated [user](user).
...
<Row>
  <Col>
    Returns all of the stores that the currently authenticated [user](user) is a member of.
