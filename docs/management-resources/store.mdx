...
<Row>
  <Col>
    ...
    Stores are owned by a single [user](management-resources/user).

    To access the store's information using a [customer token](management-resources/customer#create-customer-token),
...
<Row>
  <Col>
    ...
    Members are identified by the underlying [user](management-resources/user)'s ID.
...
## Create Store {{ tag: 'POST' }}

<Row>
  <Col>
    Creates a new store owned by the currently authenticated [user](management-resources/user).
    ...
    <Fields parameters>
      ...
      <Field name="game" type="enum" required>
        the game of the store
        <Enums>
          <Enum value="rust" />
        </Enums>
      </Field>
      <Field name="currency" type="enum" required>
        the currency of the store. Cannot be changed.
        <Enums>
          <Enum value="USD" />
          <Enum value="EUR" />
    ...
    ### Returns
    Returns the newly created [store](#the-store-object).
  </Col>
  <Col sticky>
    <Endpoints request>
        <Endpoint method="POST" url="/stores" />
    </Endpoints>
...
## Get Current User Stores {{ tag: 'GET' }}

<Row>
  <Col>
    Returns all of the stores that the currently authenticated [user](management-resources/user) is a member of.

    ### Returns
    Returns an array of [stores](#the-store-object).

  </Col>
  <Col sticky>
    <Endpoints request>
        <Endpoint method="GET" url="/stores" />
    </Endpoints>
