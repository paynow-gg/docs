...
<Row>
  <Col>
    ...
    use the [Storefront Store API](api/storefront-store).
...
<Row>
  <Col>
    Creates a new store owned by the currently authenticated [user](api/user).
...
<Row>
  <Col>
    Returns all of the stores that the currently authenticated [user](api/user) is a member of.
